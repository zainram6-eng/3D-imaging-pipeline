...
np.savetxt('Pronto-Imageur-3D/stockage/anglest.txt', (alpha_d, beta_d), fmt='%1.16e')
...
#Matrice ME
ME = ICE.dot(AE)
np.savetxt('Pronto-Imageur-3D/stockage/MEt.txt', ME, fmt='%-7.9f')
...
for k in range (N):
    ...
    del ima
    #enregistrement
    A = 'Pronto-Imageur-3D/img/It' + str(k+1) + '.bmp' 
    io.imsave(A,Im)
    # plt.figure(k)
    # io.imshow(Im)
